...
class EDL_Loss(nn.Module):
    ...
    def forward(self, logit, label, epoch_num):
        ...
        # balanced term from the original code
        loss_ce = torch.sum((2-self.prior) * one_hot_y * (torch.digamma(S) - torch.digamma(alpha))) / logit.shape[0]
        # bayes risk defined by the original paper
        # loss_ce = torch.sum(one_hot_y * (torch.digamma(S) - torch.digamma(alpha))) / logit.shape[0]
